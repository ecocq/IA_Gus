...
love.filesystem.load("fuzzylogic.lua")()
...
----------------------------------------------------------------------------------------------------------
-- Décommenter ici pour tester la qualité du parseur
----------------------------------------------------------------------------------------------------------
--print("Test parseur et mots composés")
--tprint(parseSentence("Bonjour! Aujourd'hui j'ai pris un rendez-vous sur un arc-en-ciel. arc-en-vous ?"))
--print("Test parseur ponctuation:")
--tprint(parsePonctuation("Bonjour!!! La! Mais non... Ici.. Roh; donc. Oh hey?? Oh ?! Vraiment?"))
----------------------------------------------------------------------------------------------------------
...
-- then.. don't overload it with calculation!
	...
	-- Draw the Objects
	for _, v in ipairs(Object_group) do
		if(not v.grabbed) then -- si l'objet n'est pas ramassé, on le dessine
	...
	-- Draw Gus' image
    love.graphics.draw(Gus.img, Gus.x, Gus.y)
...
-- (if you still use a main(), it'll be called before love.load()).
	...
	gauge_sociability = Gauge:new{
		...
		container_x = love.graphics.getWidth()-GAUGE_CONTAINER_WIDTH-30-40;
		container_y = love.graphics.getHeight()-GAUGE_CONTAINER_HEIGHT-30;
		caption_x = love.graphics.getWidth()-70-40;
		caption_y = love.graphics.getHeight()-35;
		label = GUS_SOCIABILITY_LABEL;
	}
	
	-- Create an instance of Gauge for the Patience dimension.
	gauge_patience = Gauge:new{
		container_imgfile = "assets/img/Gauge_Container.png";
		caption_imgfile = "assets/img/Gauge_Patience_Caption.png";
		container_x = love.graphics.getWidth()-GAUGE_CONTAINER_WIDTH-30-80;
		container_y = love.graphics.getHeight()-GAUGE_CONTAINER_HEIGHT-30;
		caption_x = love.graphics.getWidth()-70-80;
		caption_y = love.graphics.getHeight()-35;
		label = GUS_PATIENCE_LABEL;
	}
	
	-- Objects instances
	Object1 = createObject("Lettre pour Stan")
	Object_group = {Object1}
	
	
	-- NPC instances
	NPC1 = createNPC("Stan")
	NPC2 = createNPC("Marie")
	NPC3 = createNPC("Raphael")
	NPC4 = createNPC("Jean")
	NPC5 = createNPC("Postier")
	
	NPC_group = {NPC1, NPC2, NPC3, NPC4, NPC5}
	
	-- Assignation des quetes
	quest_get_tired:assignQuest(NPC4)
	quest_hit_wall:assignQuest(NPC3)
	quest_impress_girl:assignQuest(NPC2)
	...
	gauge_patience.container_img = love.graphics.newImage(gauge_patience.container_imgfile)	-- Gauge: patience (container)
	gauge_patience.caption_img = love.graphics.newImage(gauge_patience.caption_imgfile)	-- Gauge: patience (caption)
	...
	-- NPCs
	NPC1:setImage(); NPC2:setImage(); NPC3:setImage(); NPC4:setImage(); NPC5:setImage();
	...
	-- choisit aleatoirement les positions
	NPC1:setArea(); NPC2:setArea(); NPC3:setArea(); NPC4:setArea(); NPC5:setArea();
	NPC.randomePositionNew()
